...
async fn e2e_websocket_broadcast_after_ingest() {
    // Each test uses fresh Uuid::new_v4() host ids and its own AppState (so its
    // own tokio broadcast channel), so isolation is by UUID rather than by
    // truncating tables — truncates raced with parallel tests and caused FK
