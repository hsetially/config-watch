...
async fn e2e_websocket_broadcast_after_ingest() {
    ...
    db_helpers::seed_host(state.db.pool(), host_id, "ws-host", "production").await.unwrap();
    ...
    let _outcome = IngestService::ingest_change(state.db.pool(), &state.broadcast_tx, &state.snapshot_store, body).await.unwrap();
...
async fn e2e_websocket_filter_by_environment() {
    ...
    db_helpers::seed_host(state.db.pool(), host_prod, "prod-host", "production").await.unwrap();
    db_helpers::seed_host(state.db.pool(), host_dev, "dev-host", "development").await.unwrap();
    ...
    let body_prod = make_change_event_json(&host_prod, "/etc/prod/config.yaml", "modified", "ws-prod-key");
    IngestService::ingest_change(state.db.pool(), &state.broadcast_tx, &state.snapshot_store, body_prod).await.unwrap();

    // Ingest event for development host
    let body_dev = make_change_event_json(&host_dev, "/etc/dev/config.yaml", "modified", "ws-dev-key");
    IngestService::ingest_change(state.db.pool(), &state.broadcast_tx, &state.snapshot_store, body_dev).await.unwrap();
    ...
    assert!(prod_count >= 1, "Expected at least one production event to match filter");
    assert!(dev_count >= 1, "Expected at least one development event to not match filter");
}
...
async fn e2e_websocket_filter_by_severity() {
    ...
    let host_id = Uuid::new_v4();
    db_helpers::seed_host(state.db.pool(), host_id, "sev-host", "default").await.unwrap();
    ...
    IngestService::ingest_change(state.db.pool(), &state.broadcast_tx, &state.snapshot_store, body_warning).await.unwrap();
    ...
    assert!(warning_count >= 1, "Expected at least one warning event to match severity filter");
}