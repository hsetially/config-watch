...
fn make_snapshot_store() -> config_snapshot::store::SnapshotStore {
    ...
    config_snapshot::store::SnapshotStore::new(
        camino::Utf8Path::new(tmp.path().to_str().unwrap()),
    )
    .unwrap()
}
...
async fn ingest_valid_event_returns_accepted() {
    ...
    // Seed a host first
    common::db_helpers::seed_host(&pool, host_id, "ingest-host", "default").await.unwrap();

    let (broadcast_tx, _broadcast_rx) = tokio::sync::broadcast::channel(256);

    let snapshot_store = make_snapshot_store();
    let body = common::make_change_event_json(&host_id, "/etc/test.yaml", "modified", &format!("ingest-key-{}", host_id));

    let outcome = IngestService::ingest_change(&pool, &broadcast_tx, &snapshot_store, body).await.unwrap();
...
async fn ingest_duplicate_idempotency_key_returns_duplicate() {
    let pool = setup_pool().await;
    let host_id = Uuid::new_v4();
    common::db_helpers::seed_host(&pool, host_id, "dupe-host", "default").await.unwrap();
    ...
    // First ingest
    let outcome1 = IngestService::ingest_change(&pool, &broadcast_tx, &snapshot_store, body.clone()).await.unwrap();
    assert!(matches!(outcome1, IngestOutcome::Accepted { .. }));

    // Second ingest with same key
    let outcome2 = IngestService::ingest_change(&pool, &broadcast_tx, &snapshot_store, body).await.unwrap();
    assert!(matches!(outcome2, IngestOutcome::Duplicate { .. }));

    // Verify both refer to same event_id
    if let (
        IngestOutcome::Accepted { event_id: id1 },
        IngestOutcome::Duplicate { event_id: id2 },
    ) = (outcome1, outcome2)
    {
        assert_eq!(id1, id2);
    }
...
async fn ingest_broadcasts_realtime_message() {
    let pool = setup_pool().await;
    let host_id = Uuid::new_v4();
    common::db_helpers::seed_host(&pool, host_id, "broadcast-host", "default").await.unwrap();

    let snapshot_store = make_snapshot_store();
    let (broadcast_tx, mut broadcast_rx) = tokio::sync::broadcast::channel(256);

    let body = common::make_change_event_json(&host_id, "/etc/broadcast.yaml", "created", &format!("broadcast-key-{}", host_id));

    let _ = IngestService::ingest_change(&pool, &broadcast_tx, &snapshot_store, body).await.unwrap();
...
async fn ingest_touches_host_heartbeat() {
    let pool = setup_pool().await;
    let host_id = Uuid::new_v4();
    common::db_helpers::seed_host(&pool, host_id, "heartbeat-host", "default").await.unwrap();

    // Get initial heartbeat
    let initial: Option<chrono::DateTime<chrono::Utc>> = sqlx::query_scalar(
        "SELECT last_heartbeat_at FROM hosts WHERE host_id = $1"
    )
    .bind(host_id)
    .fetch_one(&pool)
    .await
    ...
    let snapshot_store = make_snapshot_store();
    let (broadcast_tx, _) = tokio::sync::broadcast::channel(256);
    let body = common::make_change_event_json(&host_id, "/etc/hb.yaml", "modified", &format!("hb-key-{}", host_id));
